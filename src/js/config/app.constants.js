const AppConstants = {
  appName: 'Find A Teacher',
  api: 'https://findateacherserver.herokuapp.com/',
  restApi: 'https://api.mlab.com/api/1/databases/jorisboschmans-mydb/collections/findateacherserver?apiKey=RQtq7YLKM48Vm5xnUTi-YdZ8iso66f5M',
  restApiNoKey: 'https://api.mlab.com/api/1/databases/jorisboschmans-mydb/collections/findateacherserver/',
  restApiOnlyKey: '?apiKey=RQtq7YLKM48Vm5xnUTi-YdZ8iso66f5M',
  storageKey: 'findATeacherIdStorageKey'
};

export default AppConstants;
